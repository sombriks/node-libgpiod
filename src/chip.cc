...
NAN_METHOD(Chip::getNumberOfLines) {
  ...
  struct gpiod_chip_info *chip_info = gpiod_chip_get_info(obj->getNativeChip());
  int ret = gpiod_chip_info_get_num_lines(chip_info);
...
NAN_METHOD(Chip::getChipName) {
  ...
  struct gpiod_chip_info *chip_info = gpiod_chip_get_info(obj->getNativeChip());
  const char *name = gpiod_chip_info_get_name(chip_info);
  gpiod_chip_info_free(chip_info);
  if (!name) {
