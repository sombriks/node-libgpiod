...
Line::Line(Chip *chip, unsigned int pin) {
  line = gpiod_chip_get_line_info(chip->getNativeChip(), pin);
...
NAN_METHOD(Line::getLineOffset) {
  ...
  int ret = gpiod_line_info_get_offset(obj->getNativeLine());
  if (-1 == ret) {
...
NAN_METHOD(Line::getLineName) {
  ...
  const char *name = gpiod_line_info_get_name(obj->getNativeLine());
  if (!name)
...
NAN_METHOD(Line::getLineConsumer) {
  ...
  const char *name = gpiod_line_info_get_consumer(obj->getNativeLine());
  if (!name)
    info.GetReturnValue().Set(Nan::Undefined());
  else
    info.GetReturnValue().Set(Nan::New<v8::String>(name).ToLocalChecked());
